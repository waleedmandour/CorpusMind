//! CorpusMind Lens desktop shell — Tauri 2 lifecycle.
//!
//! Lens is a separate installable desktop app that connects to the same
//! CorpusMind engine sidecar (same port 8765, same binary) as the main
//! CorpusMind desktop app. The difference is branding + window title;
//! the engine supervisor logic is identical.
//!
...
impl EngineSidecar {
    ...
    fn spawn(&self, app: &tauri::AppHandle) -> Result<(), SidecarError> {
        ...
        // already running on ENGINE_PORT (e.g., the main CorpusMind app
        // launched its engine). If so, connect to it instead of spawning a
        // duplicate — this prevents "address already in use" crashes when
        // both apps are installed and run simultaneously.
            ...
            .and_then(|c| c.get(&health_url).send())
        {
            if resp.status().is_success() {
                info!(
                    "engine already running on port {} (likely from main CorpusMind app) — connecting instead of spawning",
                    ENGINE_PORT
                );
                return Ok(());
            }
